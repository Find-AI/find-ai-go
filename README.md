...
```sh
go get -u 'github.com/Find-AI/find-ai-go@v0.1.0-alpha.3'
```
...
params := FooParams{
	...
	Description: findai.Null[string](),

	Point: findai.F(findai.Point{
		X: findai.Int(0),
		Y: findai.Int(1),

		// In cases where the API specifies a given type,
		// but you want to send something else, use `Raw`:
		Z: findai.Raw[int64](0.01), // sends a float
	}),
}
```
...
client.Searches.Get(context.TODO(), ...,
	// Override the header
...
`*findai.Error`. This contains the `StatusCode`, `*http.Request`, and
...
if err != nil {
	...
	panic(err.Error()) // GET "/v1/searches/{id}": 400 Bad Request { ... }
}
```
...
We also provide a helper `findai.FileParam(reader io.Reader, filename string, contentType string)`
...
params := FooNewParams{
    ID:   findai.F("id_xxxx"),
    Data: findai.F(FooNewParamsData{
        FirstName: findai.F("John"),
    }),
}
