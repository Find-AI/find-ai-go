...
func TestPeopleEnrichmentEnrichNewWithOptionalParams(t *testing.T) {
	...
	resp, err := client.PeopleEnrichment.Enrich.New(context.TODO(), findai.PeopleEnrichmentEnrichNewParams{
...
func TestPeopleEnrichmentEnrichGet(t *testing.T) {
	...
	resp, err := client.PeopleEnrichment.Enrich.Get(context.TODO(), "token")
	if err != nil {
		var apierr *findai.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
